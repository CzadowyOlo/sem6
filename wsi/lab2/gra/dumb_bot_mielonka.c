...
// void minimax(int board[5][5], int depth, bool isMax, int gracz, int best, int score, int move)
// {
...
//                         move = 10*(i+1) + j+1;
//                     }
//                     minimax(board, depth-1, !isMax, gracz, best, score, move);
    ...
    for (int i = 0; i<5; i++)
    {
        for (int j = 0; j<5; j++)
        {
            ...
            if (board[i][j]==0)
            {
                ...
                // move.
                int moveVal = minimax(depth-1, false, player);
...
int main(int argc, char *argv[])
{
  ...
  int games_count = atoi(argv[5]);
    double suma_gier = games_count;
    int changer;
    char chang[1];
 while(games_count > 0){
    if(games_count%2 == 0){
